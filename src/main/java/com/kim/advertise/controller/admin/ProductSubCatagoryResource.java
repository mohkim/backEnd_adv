...
public class ProductSubCatagoryResource {
	...
	public List<ProductSubCatagory> getAllProductSubcatagory(@PathVariable Long id) {
		ProductCatagory cat = catService.getProductCatagory(id);
	...
	@GetMapping("/cat/{id}/subcatagorybynumber")
	 public List<SubCatagoryByQuantity> getListOfCatagoryByPost(@PathVariable Long id) {
		return catSubService.getSubCatagoryByPostQuantity(id);
	}
	...
	public ResponseEntity<?> saveSubCatagory(@PathVariable Long id, @Valid @RequestBody ProductSubCatagory pSc) {
		...
		if (pc != null) {
	 	     ...
	 	     pSc.setProductCatagory(pc);
			if (catSubService.save(pSc) != null) {
